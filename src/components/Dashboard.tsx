...
export default function Dashboard() {
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeLayoutType === 'Passwords') {
        payloadString = `Username: ${username}\nPassword: ${password}\nURL: ${website}`;
      } else if (activeLayoutType === 'Credit Cards') {
        payloadString = `Holder: ${cardName}\nCard #: ${cardNumber}\nDetails: ${cardDetails}`;
      } else if (activeLayoutType === 'IDs & Passports') {
        payloadString = `Full Name: ${idName}\nDocument #: ${idNumber}\nCountry: ${idIssuingCountry}\nDate of Birth: ${idDob}\nDate of Issue: ${idIssueDate}\nExpiration Date: ${idExpiry}`;
      } else {
        payloadString = secretText;
      }
  ...
  return (
    // Rich gradient background layout modeled directly on the inspiration plate
    <div className="min-h-screen bg-gradient-to-br from-[#800c14] via-[#4a0409] to-[#0c0d14] text-slate-100 flex flex-col md:flex-row selection:bg-white selection:text-red-950 font-sans">
      ...
      {/* Sidebar Panel - Dark glass overlay scheme matching the UI left rail */}
      <aside className="w-full md:w-64 bg-black/40 backdrop-blur-md p-6 border-b md:border-b-0 md:border-r border-white/10 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-3 mb-8">
            {/* The white spider medallion token design */}
      ...
      <main className="flex-1 p-8 lg:p-12 max-w-5xl mx-auto w-full">
        <header className="mb-8">
          {/* Stark white stylized headline inspired by main display logo type */}
          ...
          <section className="lg:col-span-2 space-y-4">
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map(item => (
                <div key={item.id} className="bg-black/30 backdrop-blur-md border border-white/10 p-5 rounded-xl shadow-xl flex items-center justify-between gap-4 transition-all hover:border-white/20">
                  <div className="min-w-0 flex-1">
                    <h4 className="font-bold text-white tracking-wide uppercase text-sm">{item.title}</h4>
                    <div className="mt-2 font-mono text-xs bg-black/50 px-3 py-2 rounded border border-white/5 whitespace-pre-wrap break-all text-slate-300 shadow-inner">
                      {item.isRevealed ? (
                        // Bright blue text color for readable unlocked credentials matching the user interaction accents
                        <span className="text-cyan-400 block font-bold tracking-wide">{item.decryptedText}</span>
                      ) : (
                        <span className="text-slate-500 truncate block tracking-widest opacity-60">{item.encrypted_data}</span>
                      )}
                    </div>
                  </div>
